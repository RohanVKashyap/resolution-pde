...
def load_burger_data_from_mat(data_path1, data_path2=None, batch_size=16, res_scale = 1, split=[0.8, 0.1, 0.1]):
    """
    ...
    # Apply downsampling if res_scale > 1
    if res_scale > 1:
        # For 1D data, downsample along the spatial dimension
        # Assuming the last dimension is the spatial dimension
        X = X[:, ::res_scale]
        y = y[:, ::res_scale]
        print(f"Downsampled data shapes (scale={res_scale}): X={X.shape}, y={y.shape}")
...
def load_darcy_data_from_mat(data_path1, data_path2, res_scale = 1, batch_size=16):
    ...
    # Apply downsampling if res_scale > 1
    if res_scale > 1:
        # For 2D data, downsample both spatial dimensions
        X = X[:, ::res_scale, ::res_scale]
        y = y[:, ::res_scale, ::res_scale]
        print(f"Downsampled data shapes (scale={res_scale}): X={X.shape}, y={y.shape}")
