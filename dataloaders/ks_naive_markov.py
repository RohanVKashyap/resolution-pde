...
from utils.low_pass_filter import lowpass_filter_1d
...
class KSTrajectoryDatasetFromFile(Dataset):
    ...
    def __init__(self, filename, saved_folder, reduced_batch=1, reduced_resolution=1, 
                 reduced_resolution_t=1, use_low_pass_filter=False, lowpass_cutoff_ratio=1.0,
                 num_samples_max=-1, **kwargs):
        ...
        self._load_ks_trajectories(root_path, reduced_batch, reduced_resolution, 
                                 reduced_resolution_t, use_low_pass_filter, 
                                 lowpass_cutoff_ratio, num_samples_max)
    ...
    def _load_ks_trajectories(self, root_path, reduced_batch, reduced_resolution, 
        ...
        with h5py.File(root_path, 'r') as f:
            ...
            pde_data = pde_data[::reduced_batch, ::reduced_resolution_t, :]
            print(f"After batch/time downsampling: {pde_data.shape}")
            ...
            if reduced_resolution > 1:
                if use_low_pass_filter:
                    ...
                    # Convert to tensor
                    data_tensor = torch.from_numpy(pde_data).float()
                    
                    # Calculate cutoff ratio
                    adaptive_cutoff = (1.0 / reduced_resolution) * lowpass_cutoff_ratio
                    print(f"  Cutoff ratio: {adaptive_cutoff:.4f}")
                    
                    # Apply low-pass filter ONLY (no additional downsampling)
                    pde_data = lowpass_filter_1d(data_tensor, cutoff_ratio=adaptive_cutoff).numpy()
                else:
                    print(f"Using naive spatial downsampling (factor={reduced_resolution})")
                    pde_data = pde_data[:, :, ::reduced_resolution]
            
            print(f"After spatial processing: {pde_data.shape}")
        ...
        with h5py.File(root_path, 'r') as f:
            ...
            if reduced_resolution > 1:
                if self.use_low_pass_filter:
                    ...
                    # Calculate cutoff ratio: for reduced_resolution=2, cutoff=0.5; for 4, cutoff=0.25
                    adaptive_cutoff = (1.0 / reduced_resolution) * self.lowpass_cutoff_ratio
                    print(f"  Cutoff ratio: {adaptive_cutoff:.4f}")
            ...
            if self.x_coords is not None:
                # For low-pass filter, we keep all spatial points (no downsampling of coords)
                # For naive downsampling, we downsample the coordinates
                if not self.use_low_pass_filter and reduced_resolution > 1:
...
def ks_markov_dataset(filename, saved_folder, data_normalizer=True, 
                      use_low_pass_filter=False, lowpass_cutoff_ratio=1.0,
    ...
    print(f"Creating KS dataset with use_low_pass_filter={use_low_pass_filter}")
    if use_low_pass_filter:
