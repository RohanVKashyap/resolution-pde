    ...
    with h5py.File(root_path, 'r') as f:
        ...
        if reduced_resolution > 1:
            if use_low_pass_filter:
                ...
                _data = lowpass_filter_1d(data_tensor, cutoff_ratio=adaptive_cutoff).numpy()
            else:
                print(f"Using naive spatial downsampling (factor={reduced_resolution})")
                _data = _data[:, :, ::reduced_resolution]
...
class H5pyMarkovDataset(Dataset):
    def __init__(self, 
        ...
        with h5py.File(root_path, 'r') as f:
            ...
            # Apply same downsampling to grid (only for naive downsampling)
            if reduced_resolution > 1 and not use_low_pass_filter:
                self.grid = self.grid[::reduced_resolution]
...
def burger_markov_dataset(filename, saved_folder, data_normalizer=True, 
    ...
    """
    print(f"Creating Burger dataset with use_low_pass_filter={use_low_pass_filter}")
    if use_low_pass_filter:
        print(f"Low-pass filter cutoff ratio: {lowpass_cutoff_ratio}")
    
    # Create the full dataset with optional low-pass filtering
    full_dataset = H5pyMarkovDataset(
        filename, 
        saved_folder, 
