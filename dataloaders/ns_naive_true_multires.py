...
class NSVTrueMultiResMarkovDataset(Dataset):
    def __init__(self, 
                 ...
                 use_low_pass_filter=False,  # NEW: Use low-pass filter for downsampling
                 lowpass_cutoff_ratio=1.0,  # NEW: Cutoff ratio for low-pass filter
                 split_ratio=None,
                 random_seed=42,
                 split='train',
                 **kwargs):
        ...
        if add_res:
            ...
            if use_low_pass_filter:
                print(f"Using low-pass filter for downsampling: {use_low_pass_filter}")
        ...
        for resolution, target_samples in data_mres_size.items():
            ...
            # Further sample if target_samples is specified
            ...
            x_res = sampled_data[:, 1:-1]
            y_res = sampled_data[:, 2:]
    ...
    def _add_downsampled_data(self, saved_folder, base_resolution, add_res, add_res_samples, 
        ...
        for target_res in add_res:
            ...
            target_samples = add_res_samples.get(target_res, 100)
            samples_for_split = int(target_samples * split_ratio[split_idx])
            ...
            # Sample from split data
            np.random.seed(self.random_seed + target_res + split_idx + 10000)
            ...
            # Apply time reduction FIRST (before downsampling)
            sampled_data = sampled_data[::reduced_batch, ::reduced_resolution_t, :, :, :]
            
            # Downsample spatially
            if self.use_low_pass_filter:
                print(f"  Using low-pass filter for downsampling to resolution {target_res}")
                
                # Convert to tensor for low-pass filtering
                # Current shape: (batch, time, height, width, channels)
                sampled_tensor = torch.from_numpy(sampled_data).float()
                
                # Calculate adaptive cutoff ratio
                adaptive_cutoff = (target_res / original_spatial_size) * self.lowpass_cutoff_ratio
                print(f"  Cutoff ratio: {adaptive_cutoff:.4f}")
                
                # Rearrange to: (batch, time, channels, height, width)
                sampled_tensor = rearrange(sampled_tensor, 'b t h w c -> b t c h w')
                
                # Apply low-pass filter (this generates the lower resolution data directly)
                downsampled_data = lowpass_filter_2d(sampled_tensor, cutoff_ratio=adaptive_cutoff)
                
                # Rearrange back to: (batch, time, height, width, channels)
                downsampled_data = rearrange(downsampled_data, 'b t c h w -> b t h w c').numpy()
                
            else:
                print(f"  Using naive downsampling to resolution {target_res}")
                # Naive downsampling: take every Nth pixel
                downsampled_data = sampled_data[:, :, ::downsample_factor, ::downsample_factor, :]
            ...
            for i in range(len(x_reshaped)):
                self.x.append(x_reshaped[i])
                self.y.append(y_reshaped[i])
                downsample_method = "lowpass" if self.use_low_pass_filter else "naive"
                self.resolution_info.append(f"{target_res}_downsampled_{downsample_method}")
...
def ns_true_multires_markov_dataset(saved_folder, 
    ...
    Args:
        ...
        use_low_pass_filter: If True, use low-pass filter for downsampling. Otherwise use naive downsampling.
