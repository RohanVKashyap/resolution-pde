...
class KSTrueMultiResMarkovDataset(Dataset):
    def __init__(self, 
                 ...
                 nt=51,   # nt parameter
                 train_s=2048,  # Training data suffix (e.g., 2048)
                 reduced_batch=1, 
                 reduced_resolution_t=1, 
    ...
    def _add_downsampled_data(self, saved_folder, base_resolution, add_res, add_res_samples, 
        ...
        for target_res in add_res:
            ...
            if self.use_low_pass_filter:
                ...
                # Shape: (batch, time, spatial)
                sampled_tensor = torch.from_numpy(sampled_data).float()
                
                # Calculate cutoff ratio based on target resolution
                ...
                downsampled_data = lowpass_filter_1d(sampled_tensor, cutoff_ratio=adaptive_cutoff).numpy()
            else:
                print(f"  Using naive downsampling to resolution {target_res}")
                # Naive downsampling
                downsample_factor = original_spatial_size // target_res
                downsampled_data = sampled_data[:, :, ::downsample_factor]
...
def ks_true_multires_markov_dataset(saved_folder, 
                                   ...
                                   nte=51,
                                   nt=51,
                                   train_s=2048,  # Training data suffix
                                   data_mres_size=None,
                                   add_res=None,
