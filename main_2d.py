...
from utils.resize_utils import get_lower_resolutions, evaluate_cno_original_2d_all_resolution
from utils.naive_utils import evaluate_2d_all_resolution
...
# This was earlier cno_original_main.py (CORRECT)
...
def main(args: DictConfig):
    ...
    # Load Data
    if any(substring in args.dataset['pde'] for substring in ['ns']):      
        print('---------------------')
    ...
    # MULTI-GPU MODIFICATION: Adjust batch size for multiple GPUs
    num_gpus = torch.cuda.device_count()
    if num_gpus > 1:
        print(f"Detected {num_gpus} GPUs. Adjusting batch size from {batch_size} to {batch_size * num_gpus}")
    ...
    if not args.dataset['train_mres']:
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=effective_batch_size, shuffle=True)
        val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=effective_batch_size, shuffle=False)
        test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=effective_batch_size, shuffle=False)
    else:
        train_loader, val_loader, test_loader = create_grouped_dataloaders(
                                  train_dataset, val_dataset, test_dataset, batch_size=effective_batch_size)          
    ...
    if model_type == 'cno':
        model = instantiate(
                args.model,
                _recursive_=False,
                in_size=args.dataset['cno_train_size']).to(device)
    ...
    # UPDATE
    if 'saved_checkpoint_path' in args['dataset']:
        print('Loading from Checkpoint.')
        saved_checkpoint_path = args['dataset']['saved_checkpoint_path']
        checkpoint = torch.load(saved_checkpoint_path, map_location=device)
        model.load_state_dict(checkpoint['model_state_dict'])
        print("Loaded Model Checkpoint", saved_checkpoint_path)    

    # MULTI-GPU MODIFICATION: Wrap model with DataParallel
    ...
    # Note: For DataParallel, access the original model with .module
    ...
    wandb.init(
        ...
        config={
            ...
            'num_gpus': num_gpus,                    # ADD THIS
            'effective_batch_size': effective_batch_size,  # ADD THIS
    ...
    loss_history, val_loss_history = train(
                        ...
                        optimizer, scheduler, y_normalizer=y_normalizer, use_normalizer=use_normalizer, 
                        time=1, model_type=model_type, epochs=epochs)
    
    # Test model
    avg_l2_loss = evaluate(model=model, 
                           test_loader=test_loader, 
                           normalization_type=normalizer_type,
                           min_data=min_data, 
                           max_data=max_data, 
                           min_model=min_model, 
                           max_model=max_model, 
    ...
    # MULTI-GPU MODIFICATION: Save the underlying model state dict
...
# Superresolution Results
    ...
    if evaluation_type == 'use_resize':
        resolution_results = evaluate_cno_original_2d_all_resolution(
                        model=model,
                        ...
                        reduced_resolution_t=args.dataset['dataset_params']['reduced_resolution_t'],
                        resize_to_train=args.dataset['resize_to_train'],
                        normalization_type=normalizer_type,  
                        min_data=min_data,
                        max_data=max_data,
                        min_model=min_model,
        ...
        resolution_results = evaluate_2d_all_resolution(
                model=model,
                dataset_config=args.dataset,
                eval_dataset_target=eval_dataset_target if args.dataset['train_mres'] else None,  # This is only for multi-resolution
