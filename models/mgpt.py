...
class MoEGPTNO(nn.Module):
    def __init__(self,
        ...
        # Calculate the input size based on the node features
        ...
        # Print configurations to help with debugging
        print(f"Trunk size: {self.trunk_size}, Branch size: {self.branch_size}")
    ...
    def forward(self, g, u_p, g_u):
        ...
        # Keep track of the original shape for debugging
        # print(f"Original x shape: {x.shape}")
        
        pos = x[:,:,0:self.space_dim]
        z = pad_sequence([_g.ndata['x'] for _g in g_us]).permute(1, 0, 2) # B, T2, F
        
        # # If u_p is empty or has zero size in dimension 1
        # if u_p.size(1) == 0:
        #     # Skip concatenation, just use x as is
        #     print("u_p is empty, skipping concatenation")
        # else:
        #     # Make sure u_p is properly expanded to match x's sequence length
        #     u_p_expanded = u_p.unsqueeze(1).expand(-1, x.size(1), -1)
        #     # print(f"u_p shape after expansion: {u_p_expanded.shape}")
        #     # Concatenate along feature dimension
        #     x = torch.cat([x, u_p_expanded], dim=-1)
        ...
        # # Check the final shape before passing to MLP
        # print(f"Final x shape before MLP: {x.shape}")
        # print(f"Expected trunk size: {self.trunk_size}")
        
        # # Ensure the last dimension of x matches the trunk_size
        # if x.size(-1) != self.trunk_size:
        #     print(f"Warning: Input size {x.size(-1)} doesn't match trunk_size {self.trunk_size}")
        #     # Adjust the trunk_size dynamically if needed
        #     self.trunk_mlp.n_input = x.size(-1)
        #     self.trunk_mlp.linear_pre = nn.Linear(x.size(-1), self.trunk_mlp.n_hidden).to(x.device)
