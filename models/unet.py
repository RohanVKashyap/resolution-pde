...
# THIS VERSION JUST ADDS GROUP NORM
class UNet1d(nn.Module):
    """Source Code: https://github.com/pdebench/PDEBench/blob/main/pdebench/models/unet/unet.py"""
    def __init__(self, in_channels=3, out_channels=1, width=32, use_groupnorm=False):
        super().__init__()

        features = width
        self.encoder1 = UNet1d._block(in_channels, features, name="enc1", use_groupnorm=use_groupnorm)
        self.pool1 = nn.MaxPool1d(kernel_size=2, stride=2)
        self.encoder2 = UNet1d._block(features, features * 2, name="enc2", use_groupnorm=use_groupnorm)
        self.pool2 = nn.MaxPool1d(kernel_size=2, stride=2)
        self.encoder3 = UNet1d._block(features * 2, features * 4, name="enc3", use_groupnorm=use_groupnorm)
        self.pool3 = nn.MaxPool1d(kernel_size=2, stride=2)
        self.encoder4 = UNet1d._block(features * 4, features * 8, name="enc4", use_groupnorm=use_groupnorm)
        self.pool4 = nn.MaxPool1d(kernel_size=2, stride=2)

        self.bottleneck = UNet1d._block(features * 8, features * 16, name="bottleneck", use_groupnorm=use_groupnorm)

        self.upconv4 = nn.ConvTranspose1d(
            features * 16, features * 8, kernel_size=2, stride=2
        )
        self.decoder4 = UNet1d._block((features * 8) * 2, features * 8, name="dec4", use_groupnorm=use_groupnorm)
        self.upconv3 = nn.ConvTranspose1d(
            features * 8, features * 4, kernel_size=2, stride=2
        )
        self.decoder3 = UNet1d._block((features * 4) * 2, features * 4, name="dec3", use_groupnorm=use_groupnorm)
        self.upconv2 = nn.ConvTranspose1d(
            features * 4, features * 2, kernel_size=2, stride=2
        )
        self.decoder2 = UNet1d._block((features * 2) * 2, features * 2, name="dec2", use_groupnorm=use_groupnorm)
        self.upconv1 = nn.ConvTranspose1d(
            features * 2, features, kernel_size=2, stride=2
        )
        self.decoder1 = UNet1d._block(features * 2, features, name="dec1", use_groupnorm=use_groupnorm)
    ...
    @staticmethod
    def _block(in_channels, features, name, use_groupnorm=False):
        if use_groupnorm:
            # Use GroupNorm with 8 groups (or fewer if features < 8)
            num_groups = min(8, features)
            norm1 = nn.GroupNorm(num_groups=num_groups, num_channels=features)
            norm2 = nn.GroupNorm(num_groups=num_groups, num_channels=features)
        else:
            # Use BatchNorm
            norm1 = nn.BatchNorm1d(num_features=features)
            norm2 = nn.BatchNorm1d(num_features=features)
