...
"""Source code: https://github.com/RohanVKashyap/invariance-pde/blob/main/models/s4/s4.py"""
...
def LinearActivation(
    ...
    if activation == 'gelu_mlp':
        return nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(d_output, d_output, bias=bias),
        )
...
class SSMKernelDiag(SSMKernel):
    ...
    def forward(self, L, state=None, rate=1.0, batch_dt: float = None):
    ...
    def _setup_step(self, batch_dt = None):
...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def forward(self, state=None, rate=1.0, L=None, batch_dt=None):
        """See Kernel.forward() for argument documentation."""
    ...
    def _setup_state(self, batch_dt = None):
    ...
    def _setup_step(self, mode='dense', batch_dt = None):
        """Set up dA, dB, dC discretized parameters for stepping."""
...
class FFTConv(nn.Module):
    ...
    def __init__(
        ...
        **kernel_args,  # Arguments passed into inner convolution kernel
        ...
        if self.bidirectional and self.circular:
            raise ValueError("Bidirectional and circular not supported together")
        self.channels = channels
        self.transposed = transposed
    ...
    def forward(self, x, state=None, rate=1.0, batch_dt = None, **kwargs): # absorbs return_output and transformer src mask
        """
        ...
        k, k_state =  self.kernel(L=l_kernel, rate=rate, state=state, batch_dt=batch_dt) # (C H L) (B C H L)
        ...
        if self.circular:
            k_f = torch.fft.rfft(k) # (C H L)
            x_f = torch.fft.rfft(x) # (B H L)
        else: 
...
def get_fconv_layer(fconv_type, d_model, **kwargs):
    """Get a Fourier Convolution layer from the registry."""
    # New
    # from models.sandbox.s4_circular import FFTConvCircular
    # from models.sandbox.s4_freq_real_param import FFTConvFreqRealParam
    # from models.sandbox.s4_freq_complex_param import FFTConvFreqComplexParam
    # from models.sandbox.s4_freq_real_param_circular import FFTConvFreqRealParamCircular
    # from models.sandbox.s4_freq_complex_param_circular import FFTConvFreqComplexParamCircular

    # New
    fft_registry = {'standard': FFTConv,} 
                # 'circular': FFTConvCircular,
                # 'freq_real_param': FFTConvFreqRealParam,
                # 'freq_complex_param': FFTConvFreqComplexParam,
                # 'freq_real_param_circular': FFTConvFreqRealParamCircular,
                # 'freq_complex_param_circular': FFTConvFreqComplexParamCircular,}

    return fft_registry[fconv_type](d_model, **kwargs)


# New
from models.custom_layer import get_residual_layer
...
class S4Block(nn.Module):
    ...
    Other options are all experimental and should not need to be configured.
     - fconv_type: FFTConv type, experimental
     - inside_residual_type: residual connection after FFTConv, experimental
    """
    ...
    def __init__(
        ...
        **layer_args,  # Arguments into inner layer (e.g. FFTConv)
        ...
        self.layer = get_fconv_layer(fconv_type,
                                   d_model, transposed=False, 
                                   dropout=dropout, tie_dropout=tie_dropout, **layer_args)
        ...
        self.residual_layer = get_residual_layer(inside_residual_type, self.d_model)
    ...
    def forward(self, x, lengths=None, **kwargs): # absorbs return_output and transformer src mask
        ...
        y = y + self.residual_layer(x) #this residual is zero unless experimenting
