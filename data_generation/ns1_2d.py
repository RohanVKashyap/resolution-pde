...
def navier_stokes_2d(w0, f, visc, T, delta_t=1e-4, record_steps=1):
    print('Generating Data at Viscosity:', visc)
...
print('Setup 2d GRF successful!')

#Forcing function: 0.1*(sin(2pi(x+y)) + cos(2pi(x+y)))
t = torch.linspace(0, 1, s+1, device=device)
t = t[0:-1]

X,Y = torch.meshgrid(t, t)      # UPDATE
f = 0.1*(torch.sin(2*math.pi*(X + Y)) + torch.cos(2*math.pi*(X + Y)))

#Number of snapshots from solution
record_steps = 200
print('Number of Steps:', record_steps)
...
# #Number of snapshots from solution
# # Number of snapshots: 16 seconds ÷ 0.5 seconds = 32 snapshots
# record_steps = 32
# print('Number of Steps:', record_steps)
...
# viscosity_str = f"{viscosity:.6f}".rstrip('0').rstrip('.')
# filename_base = f'/data/user_data/rvk/ns_data/ns_{s}_{viscosity_str}'
...
# # try:
# #     filename_mat = f'{filename_base}.mat'
# #     scipy.io.savemat(filename_mat, mdict={
# #         'a': a.cpu().numpy(), 
# #         'u': u.cpu().numpy(), 
# #         't': sol_t.cpu().numpy()
# #     })
# #     print(f'Saved as MATLAB format: {filename_mat}')
    
# # except scipy.io.matlab._miobase.MatWriteError as e:
# #     print(f'MATLAB format failed ({e}), saving as HDF5...')
