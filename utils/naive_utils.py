...
from utils.plot_utils import plot_navier_stokes_examples, perform_frequency_analysis, save_numerical_results_generic, create_individual_2d_example_plots,  plot_2d_pde_examples_multiple

from utils.frequency_analysis_plot import perform_frequency_analysis_1d
...
def evaluate_1d_all_resolution(model, dataset_config, eval_dataset_target, current_res, test_resolutions, data_resolution, pde, 
    ...
    # NEW: Store ALL predictions and targets for frequency analysis
    frequency_data = {}
    ...
    for target_res in test_resolutions:
        try:
            ...
            # Load dataset at target resolution
            ...
            if eval_dataset_target:
                ...
                if 'reduced_resolution' not in dataset_config['dataset_params']:
                    eval_config['dataset_params']['reduced_resolution'] = downsample_factor
            ...
            # Initialize accumulators for frequency analysis
            all_predictions = []
            all_targets = []
            ...
            # NEW: Concatenate all batches for frequency analysis
            if analyze_frequencies:
                frequency_data[target_res] = {
                    'predictions': torch.cat(all_predictions, dim=0),  # (N, C, H)
                    'targets': torch.cat(all_targets, dim=0)           # (N, C, H)
                }
                print(f"Accumulated {frequency_data[target_res]['predictions'].shape[0]} samples for frequency analysis")
    ...
    if plot_examples and plot_data:
        ...
        plot_1d_pde_examples_multiple(plot_data, test_resolutions, pde=pde, 
                                      save_dir=prediction_plots_dir, num_examples=num_plot_examples)
    
    # NEW: Perform frequency analysis on accumulated data
    if analyze_frequencies and frequency_data and save_dir:
        print("\n" + "="*50)
        print("PERFORMING FREQUENCY ANALYSIS")
        print("="*50)
        perform_frequency_analysis_1d(frequency_data, test_resolutions, current_res, pde, frequency_analysis_dir)
...
def evaluate_2d_all_resolution(model, dataset_config, eval_dataset_target, current_res, test_resolutions, data_resolution, pde, 
                               saved_folder, reduced_batch, reduced_resolution_t, 
                               x_normalizer, y_normalizer, 
                               batch_size=8, time=1, 
                               ...
                               save_dir=None, analyze_frequencies=True):
    """
    GENERIC evaluation function for ANY 2D PDE (especially Navier-Stokes) at multiple resolutions using naive downsampling.
    ...
    if plot_examples and plot_data:
        ...
        # Create a plotting function that handles the multiple examples format
    ...
    # Perform frequency analysis if requested for 2D data
    if analyze_frequencies and frequency_analysis_data and save_dir:
        print("Performing 2D frequency analysis...")
