...
def evaluate_cno_original_1d_all_resolution(model, dataset_config, eval_dataset_target, current_res, test_resolutions, data_resolution, pde, 
                                            ...
                                            train_mres=True, use_low_pass_filter=False,
    ...
    for target_res in test_resolutions:
        try:
            ...
            if eval_dataset_target:
                ...
                if 'reduced_resolution' not in dataset_config['dataset_params']:
                    eval_config['dataset_params']['reduced_resolution'] = downsample_factor   # UPDATE
            ...
            # eval_config['dataset_params']['s'] = target_res            # Load at test resolution (UPDATE)
            eval_config['dataset_params']['data_normalizer'] = False   # No normalization - apply manually
            ...
            with torch.no_grad():
                for batch_idx, batch_data in enumerate(test_loader):
                    ...
                    if resize_to_train:
                        ...
                        # UPDATE: Resize predictions back to target test resolution
                        if train_mres and use_low_pass_filter:
                           batch_pred_normalized_final = batch_pred_normalized 
                        else:     
...
# THIS IS PERFECT: THE ABOVE ONE IS ADAPTED FOR USING LOW-PASS FILTER WITH CNO FOR MULTI-RESOLUTION TRAINING
# def evaluate_cno_original_1d_all_resolution(model, dataset_config, eval_dataset_target, current_res, test_resolutions, data_resolution, pde, 
#                                             saved_folder, reduced_batch, reduced_resolution_t, 
#                                             normalization_type='minmax',
#                                             min_data=None, max_data=None, min_model=None, max_model=None,
