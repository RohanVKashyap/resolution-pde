...
def main(args: DictConfig):
    ...
    if model_type == 'cno1d':
        model = instantiate(
                args.model,
                _recursive_=False,
                size=args.dataset['cno_train_size']).to(device)    # in_size: UPDATE
    ...
    # checkpoint = torch.load('checkpoints/ffno1d/ks_5537836.pt', map_location=device)
    # model.load_state_dict(checkpoint['model_state_dict'])
    ...
    os.environ['WANDB__SERVICE_WAIT'] = '300'     # UPDATE
    ...
    if evaluation_type == 'use_resize':
        resolution_results = evaluate_cno_original_1d_all_resolution(
                        model=model,
                        ...
                        time=1,
                        model_type=model_type,
                        train_mres=args.dataset['train_mres'],
                        use_low_pass_filter=args.dataset['dataset_params']['use_low_pass_filter'],
                        device='cuda',
                        plot_examples=True,
